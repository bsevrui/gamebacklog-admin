...
import { of } from 'rxjs';
import { CreatePlatform } from 'src/app/core/interfaces/platform/create-platform';
...
export class AddPage implements OnInit {
  ...
  create() {
    if (this.name && this.releaseDate) {
      const platformData: CreatePlatform = {
        name: this.name,
      ...
      this.apiService.createPlatform(platformData).pipe(
        catchError((error) => {
          this.localizationService.translate(['WARNING_GENERIC', 'WARNING_PLATFORM_ALREADYCREATED']).subscribe(async (values) => {
            let errorMsg = values['WARNING_GENERIC'];

            if (error?.error?.message == 'platform already registered') {
              errorMsg = values['WARNING_PLATFORM_ALREADYCREATED'];
            }
        ...
        if (res) {
          this.localizationService.translate(['TOAST_PLATFORM_CREATED']).subscribe(async (values) => {
            this.router.navigate(['/platforms']).then(() => {
              window.location.reload();
            });
            await this.presentToast(values['TOAST_PLATFORM_CREATED'], 'success');
          });
        }
      })
    } else {
